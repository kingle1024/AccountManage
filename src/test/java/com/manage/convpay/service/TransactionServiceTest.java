...
import static com.manage.convpay.type.AccountStatus.IN_USE;
import static com.manage.convpay.type.TransactionResultType.F;
import static com.manage.convpay.type.TransactionResultType.S;
import static com.manage.convpay.type.TransactionType.CANCEL;
...
class TransactionServiceTest {
   ...
   void deleteFailed_alreadyUnregistered(){
        //given
        AccountUser pobi = AccountUser.builder()
                .name("Pobi").build();
        pobi.setId(12L);
    ...
    void succuessCancelBalance(){
        ...
        Account account = Account.builder()
                ...
                .balance(10000L)
                .accountNumber("1000000012").build();
        Transaction transaction = Transaction.builder() // 원래 거래를 찾
                .account(account)
                .transactionType(USE)
                .transactionResultType(S)
                .transactionId("transactionId")
    ...
    void cancelTransaction_AccountNotFond(){
        //given
        // 에러에 걸리지 않게 transaction만 띄워준다.
        given(transactionRepository.findByTransactionId(anyString()))
    ...
    @Test
    @DisplayName("해당 계좌 없음 - 잔액 사용 취소 실패 ")
    void cancelTransaction_TrasactionNotFound(){
        //given
        given(transactionRepository.findByTransactionId(anyString()))
                .willReturn(Optional.empty());
        //when
    ...
    void cancelTransacion_TransactionAccountUnMatch(){
        //given
        AccountUser user = AccountUser.builder()
                .name("Pobi").build();
        user.setId(12L);
        ...
        Transaction transaction = Transaction.builder()
                ...
                .transactionId("transactionId")
                .transactedAt(LocalDateTime.now())
                .amount(CANCEL_AMOUNT).build();
        given(transactionRepository.findByTransactionId(anyString()))
                .willReturn(Optional.of(transaction));
        given(accountRepository.findByAccountNumber(anyString()))
                .willReturn(Optional.of(accountNotUse)); // 그냥 account주면 에러남
    ...
    void cancelTransaction_CancelMustFully(){
        ...
        Transaction transaction = Transaction.builder()
                ...
                .transactionId("transactionId")
                .transactedAt(LocalDateTime.now())
                .amount(CANCEL_AMOUNT + 1000L).build();
        given(transactionRepository.findByTransactionId(anyString()))
                .willReturn(Optional.of(transaction));
        given(accountRepository.findByAccountNumber(anyString()))
                .willReturn(Optional.of(account)); // 그냥 account주면 에러남
    ...
    void cancTransaction_TooOldOrder(){
        //given
        AccountUser user = AccountUser.builder()
                .name("Pobi").build();
        user.setId(12L);
